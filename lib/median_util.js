...
const findStrokeMedian = (stroke) => {
	...
	if (vertices[natural_end][1] < artifact_threshold || vertices[natural_start][1] < artifact_threshold) {
		...
		if (best_leaf !== null && vertices[best_leaf][0] > vertices[fold][0] + size * 0.1) {
			...
			// Together these ensure we only reroute genuine 讠-type 横折提 strokes, not
			// 竖钩 or 丿 strokes whose entry cap happens to be near y=0.
			const segLen = (nodes) =>
				nodes.reduce(
	...
	// Extend path to include significant off-path leaf branches (e.g., ㇉ hairpin turns).
	// After the main path is chosen, any skeleton leaf not on the path represents a missed
	// branch of the stroke. If the branch is long enough to cause a visible cutoff when
	// animated with stroke-width 128 (half-width = 64 units), insert a U-detour at its
	// junction with the main path so the animation covers the full stroke outline.
	const HALF_STROKE_WIDTH = 64;
	const pathSet = new Set(path);
	const missedLeaves = allLeaves.filter(
		(n) => !pathSet.has(n) && vertices[n][1] >= artifact_threshold,
	);
	for (const leaf of missedLeaves) {
		// BFS from leaf until we reach a node already on the main path.
		const bfsPrev = { [leaf]: null };
		const bfsQueue = [leaf];
		let junction = null;
		let bfsIdx = 0;
		while (bfsIdx < bfsQueue.length && junction === null) {
		...
		// Reconstruct branch: [leaf, …, junction].
		const branch = [];
		let cur = junction;
		while (cur !== null) {
			branch.unshift(cur);
		...
		for (let i = 0; i < branch.length - 1; i++) {
			branchLen += Math.sqrt(Point.distance2(vertices[branch[i]], vertices[branch[i + 1]]));
		}
		if (branchLen < HALF_STROKE_WIDTH) continue; // Too short to cause a visible cutoff.

		const junctionIdx = path.indexOf(junction);
		const jToLeaf = branch.slice().reverse(); // [junction, …, leaf]

		if (junctionIdx === 0) {
			// Junction is the path start: prepend the branch so animation starts at the leaf.
			path = branch.concat(path.slice(1));
		} else if (junctionIdx === path.length - 1) {
			// Junction is the path end: append the branch so animation ends at the leaf.
			path = path.slice(0, -1).concat(jToLeaf);
		} else {
			// Junction is mid-path: insert a U-detour (junction→leaf→junction) in-place.
			const detour = jToLeaf.concat(branch.slice(1)); // [junction,…,leaf,…,junction]
			path = path.slice(0, junctionIdx).concat(detour).concat(path.slice(junctionIdx + 1));
		}

		pathSet.clear();
		path.forEach((n) => pathSet.add(n));
	}
