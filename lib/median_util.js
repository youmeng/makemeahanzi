...
const findStrokeMedian = (stroke) => {
	...
	// If a sub-baseline vertex exists (y < -10) that is not already a natural
	// path endpoint, findLongestShortestPath missed the hook branch. Use the
	// deepest vertex (not just leaves — the bend bottom may be a non-leaf node)
	// and route: highestEnd → deepestVertex → otherEnd.
	const naturalEndSet = new Set([natural_start, natural_end]);
	const subBaselineVertices = Object.keys(adjacency)
		.map(Number)
		.filter((n) => vertices[n][1] < -10 && !naturalEndSet.has(n));
	let artifact_rerouted = false;
	if (subBaselineVertices.length > 0) {
		const hookVertex = subBaselineVertices.reduce((a, b) => (vertices[a][1] < vertices[b][1] ? a : b));
		const highestEnd = vertices[natural_start][1] >= vertices[natural_end][1] ? natural_start : natural_end;
		const otherEnd = highestEnd === natural_start ? natural_end : natural_start;
		const highToHook = findPathBetweenNodes(adjacency, highestEnd, hookVertex);
		const hookToOther = findPathBetweenNodes(adjacency, hookVertex, otherEnd);
		if (highToHook && hookToOther && highToHook.length > 1 && hookToOther.length > 1) {
			path = highToHook.concat(hookToOther.slice(1));
	...
	if (
		!artifact_rerouted &&
		(vertices[natural_end][1] < artifact_threshold || vertices[natural_start][1] < artifact_threshold)
	) {
	...
	let points = path.map((i) => vertices[i]);

	// Extend path to polygon's deepest point if the medial axis terminates
	// significantly above the actual polygon tip (the axis is always interior,
	// so it can't fully reach a narrow fold/hook tip).
	// Threshold: 4px when the path was rerouted (catches small sub-baseline
	// gaps like 农's 14px), ~8% of size (~82px) otherwise (catches wide folds
	// like 讨's ㇊ at 117px gap, while ignoring the normal stroke-half-width
	// gap on straight strokes which is typically < 50px).
	{
		const polyMinPt = polygon.reduce((a, b) => (b[1] < a[1] ? b : a));
		let hookIdxInPoints = 0;
		let hookMinY = points[0][1];
		for (let pi = 1; pi < points.length; pi++) {
			if (points[pi][1] < hookMinY) {
				hookMinY = points[pi][1];
				hookIdxInPoints = pi;
			}
		}
		const extendThreshold = artifact_rerouted ? 4 : size * 0.08;
		if (polyMinPt[1] < hookMinY - extendThreshold) {
			points = [...points.slice(0, hookIdxInPoints + 1), polyMinPt, ...points.slice(hookIdxInPoints + 1)];
		}
	}
