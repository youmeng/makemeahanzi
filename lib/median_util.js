...
const findStrokeMedian = (stroke) => {
	...
	// If we rerouted to a sub-baseline hook vertex, the Voronoi medial axis may
	// terminate a few pixels above the actual polygon tip (the axis is interior to
	// the polygon, so it can't reach the very tip). Extend the path by inserting
	// the polygon's deepest vertex right after the deepest path point so the
	// animation fully covers the hook bottom.
	if (artifact_rerouted) {
		const polyMinPt = polygon.reduce((a, b) => (b[1] < a[1] ? b : a));
		let hookIdxInPoints = -1;
		let hookMinY = 0;
		...
		if (hookIdxInPoints >= 0 && polyMinPt[1] < hookMinY - 4) {
