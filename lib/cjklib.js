...
const parseUnicodeStr = (str) => String.fromCodePoint(parseInt(str.substr(2), 16));
	...
	new Promise((resolve, reject) => {
		if (Meteor.isServer) {
			...
			$.get(filename, (data, code) => {
				if (code !== "success") {
					const error = new Error(`Failed to load ${filename}: ${code}`);
					console.error(error);
					reject(error);
				} else {
					resolve(data);
...
const fillVariants = (simplified, traditional, variants) => {
	return variants.then((rows) => {
		rows.map((row) => {
			if (
				(row[1] !== "kSimplifiedVariant" && row[1] !== "kTraditionalVariant") ||
				row[0] === row[2] ||
				row[0] === "U+2B5B8"
			) {
...
const fillGB2312 = (data, result) => {
	Array.from(data).forEach((character) => {
		if (character === "\n") return;
		assert(character.length === 1, `Expected single character, got: ${character}`);
		const codepoint = character.codePointAt(0);
		// GB2312 includes characters from multiple Unicode blocks:
		// - CJK Unified Ideographs (0x4e00-0x9fff)
		// - CJK Unified Ideographs Extension A (0x3400-0x4dbf)
		// - CJK Compatibility Ideographs (0xf900-0xfaff)
		// Skip non-CJK characters (like newlines that weren't caught above)
		if (codepoint < 0x3400) return;
		result[character] = true;
	});
	// The assertion for exactly 6763 characters might be too strict if the file
	// format has changed. Log a warning instead of crashing.
	const count = Object.keys(result).length;
	if (count !== 6763) {
		console.warn(`GB2312: Expected 6763 characters, got ${count}`);
	}
};
...
const cleanupCJKLibData = () => {
	...
	Object.keys(characters.decomposition).map((character) => {
		...
		characters.decomposition[character] = Array.from(decomposition)
			.map(convert_astral_characters)
			.map(radical_to_character)
			.join("");
	});
	for (let i = 1; i <= 214; i++) {
...
Meteor.startup(() => {
	...
	const radical_isolated_characters = readFile("cjklib/kangxiradicalisolatedcharacter.csv").then(getCJKLibRows);

	// Jun Da's character frequency data, used only for prioritization.
	const frequencies = readFile("junda/character_frequency.tsv").then(getFrequencyRows);
	...
	cjklib.promise = Promise.all([
		...
		fillRadicalToCharacterMap(locale, radical_equivalent_characters, cjklib.radicals.radical_to_character_map),
		fillVariants(cjklib.characters.simplified, cjklib.characters.traditional, variants),
	...
	cjklib.promise.catch((error) => {
		console.error("CJKLib initialization error:", error);
