...
const checkBridge = (bridge) => {
	assert(bridge !== undefined && bridge !== null, "Bridge is null or undefined");
	assert(bridge.length === 2, `Bridge should have 2 points, got ${bridge.length}`);
	assert(bridge[0] !== undefined, "Bridge[0] is undefined");
	assert(bridge[1] !== undefined, "Bridge[1] is undefined");
	assert(Point.valid(bridge[0]), `Bridge[0] is not a valid point: ${JSON.stringify(bridge[0])}`);
	assert(Point.valid(bridge[1]), `Bridge[1] is not a valid point: ${JSON.stringify(bridge[1])}`);
	assert(!Point.equal(bridge[0], bridge[1]), `Bridge endpoints are equal: ${JSON.stringify(bridge[0])}`);
};
...
const getBridges = (endpoints, classifier) => {
	...
	for (let i = 0; i < corners.length; i++) {
		...
		if (j === undefined || j === null) {
			console.error(`Invalid matching index for corner ${i}: j = ${j}`);
			continue;
		}
		if (j < 0 || j >= corners.length) {
			console.error(`Out of bounds matching index for corner ${i}: j = ${j}, corners.length = ${corners.length}`);
			continue;
		}
		...
		// Validate corners before creating bridge
		if (!corners[i] || !corners[i].point) {
			console.error(`Invalid corner at index ${i}:`, corners[i]);
			continue;
		}
		if (!corners[j] || !corners[j].point) {
			console.error(`Invalid corner at index ${j}:`, corners[j]);
			continue;
		}

		result.push([Point.clone(corners[i].point), Point.clone(corners[j].point)]);
	}
	result.forEach(checkBridge);
	return result;
};
...
const extractStroke = (paths, endpoint_map, bridge_adjacency, log, extracted_indices, start, attempt_one) => {
	...
	while (true) {
		...
		if (bridge_adjacency.hasOwnProperty(key)) {
			...
			const options = bridge_adjacency[key].sort((a, b) => angle(endpoint.index, a) - angle(endpoint.index, b));
		...
		if (Point.equal(current, start)) {
			if (self_intersecting) {
				log.push({ cls: "error", message: "Extracted a self-intersecting stroke." });
			}
...
const extractStrokes = (paths, endpoints, bridges, log) => {
	...
	log.push({ cls: "error", message: "Stroke extraction missed some path segments." });
	return strokes;
};
