...
const migrateOldGlyphSchemaToNew = (glyph) => {
	...
	assert(glyph.manual && glyph.manual.verified !== undefined, `Glyph ${character} was not verified.`);
...
const dumpToNewSchemaJSON = () => {
	...
	const pwd = getPWD();
	console.log("Starting export to dictionary.txt and graphics.txt...");
	...
	runMigration(
		(glyph) => {
			const result = dumpGlyph(dictionary, graphics)(glyph);
			if (result !== undefined || glyph.stages.verified) {
				exported++;
			}
		},
		() => {
			dictionary.end();
			graphics.end();
			console.log(`Export complete. Exported ${exported} verified glyphs to dictionary.txt and graphics.txt`);
		}
	);
};
...
const exportSVGs = () => {
	...
	runMigration(
		(glyph) => {
			// Only export glyphs that have been fully verified with order data
			if (!glyph.stages.order || !glyph.stages.verified) {
				skipped++;
				return;
			}
			...
			// Debug: Log the first export to verify data
			if (exported === 0) {
				console.log(`Exporting first glyph: ${glyph.character} (U+${codepoint.toString(16).toUpperCase()})`);
				console.log(`  Strokes: ${strokes.length}, Medians: ${medians.length}`);
			}

			const animationData = getAnimationData(strokes, medians);
			const raw = SSR.render("animation", animationData);

			// Debug: Check if SSR produced output
			if (exported === 0) {
				console.log(
					`  Animation data:`,
					JSON.stringify({
						animationCount: animationData.animations.length,
						strokesCount: animationData.strokes.length,
						firstStroke: animationData.strokes[0].substring(0, 50),
					})
				);
				console.log(`  Raw SVG length: ${raw.length} bytes`);
				console.log(`  Raw SVG preview:`, raw.substring(0, 200));
			}

			const svg = raw.replace(/\n  /g, "\n").split("\n").slice(1, -2).join("\n");
			const filepath = path.join(directory, `${codepoint}.svg`);
			fs.writeFileSync(filepath, svg);

			// Debug: Verify file was written
			if (exported === 0) {
				const fileSize = fs.statSync(filepath).size;
				console.log(`  Written to: ${filepath} (${fileSize} bytes)`);
			}

			exported++;
		},
		() => {
			console.log(`SVG export complete. Exported: ${exported}, Skipped: ${skipped}`);
		}
	);
};
...
const runMigration = (per_glyph_callback, completion_callback) => {
	...
	if (per_glyph_callback) {
		const codepoints = Glyphs.find({}, { fields: { codepoint: 1 }, sort: { codepoint: 1 } }).fetch();
...
Meteor.methods({
	export: () => {
		console.log("Export method called from client");
		cjklib.promise.then(Meteor.bindEnvironment(dumpToNewSchemaJSON)).catch(console.error.bind(console));
		return "Export started - check server console for progress";
	},
	exportSVGs: () => {
		console.log("exportSVGs method called from client");
		exportSVGs();
		return "SVG export started - check server console for progress";
	},
	loadFromOldSchemaJSON: (filename) => {
		console.log(`loadFromOldSchemaJSON method called from client with filename: ${filename}`);
		cjklib.promise.then(Meteor.bindEnvironment(() => loadFromOldSchemaJSON(filename))).catch(console.error.bind(console));
		return `Loading from ${filename} - check server console for progress`;
	},
});
