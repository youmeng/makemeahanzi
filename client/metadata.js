...
Template.metadata.helpers({
	...
	items() {
		...
		if (cjklib.radicals.radical_to_index_map.hasOwnProperty(glyph.character)) {
			...
			if (glyph.character !== primary) {
				...
				result[0].extra = `<a class="link" href="#${primary}">` + `${result[0].extra} ${primary}</a>`;
			}
		}
		return result;
	},
	...
	references() {
		...
		return [
			{
				href: "http://www.archchinese.com/chinese_english_dictionary.html" + `?find=${character}`,
				label: "Arch Chinese",
			},
			{
				href: "https://www.mdbg.net/chinese/dictionary?page=worddict" + `&wdrst=0&wdqb=${character}`,
				label: "MDBG",
			},
			{
			...
			{
				href: "http://www.yellowbridge.com/chinese/character-etymology.php" + `?zi=${character}`,
				label: "YellowBridge",
			},
		];
	},
