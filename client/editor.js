...
const changeGlyph = (method, argument) => {
	...
	Meteor.call(method, argument, function (error, data) {
		...
		if (!data) {
			console.warn(`Method ${method} returned no glyph. No more glyphs match the criteria.`);
			alert(`No more glyphs found matching the criteria for ${method}.`);
			return;
		}
		if (!data.character) {
			console.error("Method returned invalid glyph (no character property):", data);
			assert(false, "Invalid glyph data: missing character property");
			return;
		}
